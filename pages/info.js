	...
	const pageContent = sortedArray.map((section) => {
		...
		switch(section.title) {
			case "Specialbeställningar och förlovningsringar":
				return <SpecialOrdersSection text={textArray} images={imageArray} title={section.title}/>

			case "Tillverkning":
				return <ProductionSection text={textArray} images={imageArray} title={section.title}/>

			case "Webbutveckling -  bygget av denna webshop":
				return <WebDevSection text={textArray} images={imageArray} title={section.title}/>

			case "Skötselråd":
				return <ProductCareSection text={textArray} title={section.title}/>
